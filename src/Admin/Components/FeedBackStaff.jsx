...
const FeedBackStaff = () => {
  ...
  useEffect(() => {
    const fetchLeaveReports = async () => {
      ...
      try {
        const response = await axios.get(
          "http://localhost:8000/api/replystaff/"
        );
        ...
        toast.error("Error fetching leave reports."); // Show error toast
      } finally {
    ...
    fetchLeaveReports();
    // const socket = new WebSocket("ws://127.0.0.1:8000/ws/staff_feedback/");
  ...
  const handleReplyClick = (report) => {
    ...
    setReply(report.reply || ""); // Populate the reply if it exists
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8000/api/replystaff/${selectedReport.id}/`,
        { reply }
      );
      if (response.status === 200) {
        ...
        toast.success("Reply submitted successfully!"); // Show success toast
      }
    } catch (error) {
      console.error("Error updating leave status:", error);
      toast.error("Error submitting reply."); // Show error toast
    } finally {
      setLoading(false); // End loading
    }
  };
    ...
    <div className="root">
      ...
      <div className="d-lg-flex d-md-block d-sm-block">
        ...
        <div className="dash flex-grow-1 p-3">
          <h2>FeedBack Reports (Staff)</h2>
          <br />
          <div className="dash-container">
            <Card className="mt-3" style={{ width: "100%" }}>
              <Card.Header>
                <Card.Title>Feedback Reports (Staff)</Card.Title>
              </Card.Header>
              <Card.Body>
                {error && (
